...
export function envInt(key: EnvIntKey, defaultValue?: number): number {
    const value = ifNotUndef(process.env[key], value => parseInt(value, 10)) ?? defaultValue;
    if (value === undefined) {
        throw new Error(`$${key} not present in environment`);
    }
    return value;
...
export function envFlag(key: EnvFlagKey, defaultValue?: boolean): boolean {
    const value = ifNotUndef(process.env[key], value => Boolean(value)) ?? defaultValue;
    if (value === undefined) {
        throw new Error(`$${key} not present in environment`);
    }
    return value;
...
export function envStrings(key: EnvStringsKey, defaultValue?: string[], separator: string = ","): string[] {
    const value = ifNotUndef(process.env[key], value => value.split(separator)) ?? defaultValue;
    if (value === undefined) {
        throw new Error(`$${key} not present in environment`);
    }
    return value;
